...
export default function Section1() {
  ...
  const carouselItems = timelineData.map((item, index) => (
    <div
      key={index}
    ...
    <section className="bg-[#FFFFFF] w-full px-5 lg:max-w-7xl mx-auto py-16">
      ...
      >
        {mounted ? (
          ...
          // Skeleton shown during SSR — same height to avoid layout shift
          <div className="flex gap-6 overflow-hidden">
            {timelineData.slice(0, 3).map((item, i) => (
              <div key={i} className="flex flex-col items-center w-[300px]">
                <div className="rounded-lg bg-gray-100 h-[400px] w-[300px]" />
                <div className="pt-10 text-center">
                  <div className="w-16 h-5 mx-auto bg-gray-100 rounded" />
                </div>
              </div>
            ))}
          </div>
